...
| [git-flow start-task](./start-task) | **从当前分支创建任务子分支** |

### 日常工作

| 命令 | 说明 |
|------|------|
| [git-flow commit](./commit) | 智能提交，自动添加前缀（支持子分支） |
| [git-flow sync](./sync) | 同步代码（子分支从父分支，集成分支从 develop） |
| [git-flow wip](./wip) | 保存工作进度 |
| [git-flow status](./status) | 查看分支状态和子分支列表 |

### 完成工作

| 命令 | 说明 |
|------|------|
| [git-flow finish](./finish) | 完成并合并分支（子分支→父分支，集成分支→develop） |
...
git-flow start-feat user-auth        # 创建集成分支
git-flow start-task login-page       # 创建任务子分支 feat/user-auth-t01
git-flow commit "实现登录页面"        # 提交
git-flow finish                      # 合并子分支到父分支

git-flow start-task auth-service     # 创建下一个任务子分支
git-flow commit "实现认证服务"
git-flow finish                      # 合并子分支到父分支

git-flow finish                      # 合并集成分支到 develop
