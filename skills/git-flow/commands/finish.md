...
| release（无 `-t` 后缀） | 提示使用 git-flow publish |
| develop/master | 禁止操作 |
...
**注意：** 子分支合并不推送远程（子分支是本地的）。用户需要在父分支上手动 `git-flow sync` 同步 develop，或最终 `git-flow finish` 合并到 develop。
...
================================================================
 ...
 下一步:
   git-flow start-task — 创建下一个任务分支
   git-flow status     — 查看父分支状态
   git-flow finish     — 完成父分支（合并到 develop）
================================================================
```
...
- [git-flow start-task](./start-task) - 创建任务子分支
- [git-flow commit](./commit) - 智能提交
- [git-flow sync](./sync) - 同步代码
