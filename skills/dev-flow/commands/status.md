...
如果不存在，提示 "未找到工作流状态文件。使用 dev-flow run <workflow> 启动工作流。"
...
优先检查用户项目 `.dev/workflows/${workflow.mode}.md`（自定义覆盖，通常不存在），不存在则用包内置的 `workflows/${workflow.mode}.md`（相对于 Skill Base directory）。
...
================================================================
 ...
 下一步:
   ...
   git-flow finish  — 合并到 <base_branch>
```

### 5. 已沉淀工作流（如果执行了 dev-flow list 或用户询问）
