...
所有 Agent 产出的工程文档（PRD、ARCHITECTURE、PLAN 等）遵循以下原则：

### 结构优于代码

文档关注**做什么、为什么这样做**，而非具体代码和文件路径。
...
用 Mermaid 图表替代文字描述结构关系。常用图表类型：

| 视角 | 图表类型 | 用途 |
|------|---------|------|
| 系统结构 | `graph` / `flowchart` | 模块划分、组件关系、系统边界 |
| 数据模型 | `erDiagram` | 实体关系、领域模型 |
| 行为交互 | `sequenceDiagram` | 模块间调用链、用户操作流程 |
| 状态变迁 | `stateDiagram-v2` | 对象生命周期、流程状态 |
| 用例 | `flowchart` | 用户场景、业务流程 |

### 软件工程思维

- **领域驱动** — 先识别领域概念和业务规则，再映射到技术方案
