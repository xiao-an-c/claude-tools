...
name: feat
display_name: "功能开发"
description: "旗舰级功能开发：产品讨论 + 架构设计 + 测试设计 + 规划 + 开发，自适应复杂度。"
category: code-change
argument_hint: "[--git|--no-git] <功能描述>"
defaults:
  use_git: true
  base_branch: develop
  branch_type: feat
...
创建 `.dev/plan/<branch-name>/` 目录，生成 config.yml、PRD.md、TEST-DESIGN.md、ARCHITECTURE.md、PLAN.md、[TECH-DESIGN.md]、TASK-LOG.md、ACCEPTANCE.md。
...
| 记录 | `dev-recorder` | sonnet | 后台 spawn（不阻塞） |
...
Type: agent-loop
Agents: dev-developer (up to 3 parallel)
Model: sonnet
Condition: loop while tasks remain
...
Condition: use_git=true AND complexity=complex
...
# 合并子分支
git merge <type>/<slug>-t<NN> --no-ff -m "Merge <type>/<slug>-t<NN>: ${task_title}"
...
**上下文保护：** 如果上下文使用超过 60%，输出进度摘要并建议 `dev-flow resume`。
...
**生成验收说明书：** 从 TASK-LOG.md 和 `git log --oneline <base_branch>..HEAD`（如果 use_git=true）提取信息，生成 `.dev/plan/${branch_name}/ACCEPTANCE.md`。
...
```bash
git add .dev/plan/${branch_name}/ACCEPTANCE.md
...
================================================================
 功能: ${description}
 复杂度: ${complexity_label}
 分支: ${branch_name} (或 "无 git 模式")
 任务: 完成 X/Y
 失败: Z
 验证: <通过/部分通过>
 产物: .dev/plan/${branch_name}/

 下一步:
   验收测试  -- 按 ACCEPTANCE.md 确认功能
   dev-flow status   -- 查看工作流状态
   git-flow finish   -- 合并到 ${base_branch} (需 use_git=true)
================================================================
```
