...
场景驱动的多 Agent 协作开发工作流。通过 `dev-flow run` 通用执行器读取工作流定义，机械执行每个步骤。

## 架构

```
dev-flow run <workflow> [--git|--no-git] <描述>
    ↓
...
**工作流即知识** — 工作流定义存储在 `.dev/workflows/` 目录中，每个工作流是一个独立的 Markdown 文件。`dev-flow run` 作为通用执行器，机械地读取工作流定义并执行其中的步骤。新场景只需新增一个工作流文件。

## 内置工作流

通过 `dev-flow run <workflow-name>` 调用：

| 工作流 | 用途 | 步骤 | Git | 预期耗时 |
|--------|------|------|-----|---------|
| `dev-flow run patch` | 小修小补，已知原因 | 4 | 无 | 2-3 min |
| `dev-flow run fix` | Bug 修复，需诊断 | 10 | `fix/*` | 5-10 min |
| `dev-flow run feat` | 新功能开发 | 13 | `feat/*` | 15-60 min |
| `dev-flow run refactor` | 代码重构 | 11 | `refactor/*` | 10-30 min |
| `dev-flow run hotfix` | 线上紧急修复 | 8 | `hotfix/*` | 3-5 min |
| `dev-flow run auto` | 即兴编排，动态设计 | 6 | 视流程 | 视流程 |
...
| 命令 | 用途 |
|------|------|
| [dev-flow review](./review) | 代码审查（只读，无 Git） |
| [dev-flow discuss](./discuss) | 架构讨论/开会（只读，无 Git） |
| [dev-flow investigate](./investigate) | Bug 排查（只读，无 Git） |
| [dev-flow status](./status) | 查看当前工作流状态和进度 |
| [dev-flow resume](./resume) | 恢复中断的工作流 |

## 通用执行器

[dev-flow run](./run) 是所有工作流的统一入口。它读取工作流定义文件，机械执行每个步骤：
...
| dev-flow-architect | opus | 工作流设计 | auto |
...
```
dev-flow investigate 排查 bug -> 建议用 dev-flow run fix 修复
dev-flow discuss 讨论方案 -> 建议用 dev-flow run feat 实现
dev-flow review 审查代码 -> 建议用 dev-flow run refactor 改进
```
...
- [git-flow start-feat](../git/start-feat) - 手动创建功能分支
- [git-flow commit](../git/commit) - 智能提交
