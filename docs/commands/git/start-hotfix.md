...
```bash
git-flow start-hotfix [description]
```
...
💡 提示: 修复完成后使用 git-flow finish 合并并发布
```
...
```bash
# 1. 开始紧急修复
git-flow start-hotfix payment-crash

# 2. 修复并提交
git-flow commit 修复支付崩溃问题
...
- [git-flow start-fix](./start-fix) - 普通 Bug 修复
